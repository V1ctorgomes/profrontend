...
export function StockPanel() {
  ...
  async function handleAddSize(e: React.FormEvent) {
    ...
    try {
      ...
      const size = sizeForm.size;
      setSizeModalOpen(false);
      toast.success('Tamanho cadastrado', `Tamanho ${size} adicionado ao estoque.`);
      await load();
    } catch (err) {
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={openSizeModal} className={btnSecondaryClass}>
              <Plus className="h-4 w-4" />
              Adicionar tamanho
            </button>
            <button type="button" onClick={openMovementModal} className={btnPrimaryClass}>
              <Plus className="h-4 w-4" />
              Movimentação
            </button>
          </div>
      ...
      <FormModal
        ...
        description="Cadastre um tamanho e quantidade inicial para um produto."
        size="md"
      >
        <form onSubmit={handleAddSize} className="space-y-3">
          <select
            ...
            onChange={(e) => setSizeForm((f) => ({ ...f, productId: e.target.value }))}
            required
          >
          ...
          <input
            ...
            onChange={(e) => setSizeForm((f) => ({ ...f, quantity: e.target.value }))}
            required
          />
      ...
      >
        <form onSubmit={handleMovement} className="space-y-3">
          ...
          <input
            ...
            onChange={(e) => setMovementForm((f) => ({ ...f, size: e.target.value }))}
            required
          />
          <select
            className={selectClass}
